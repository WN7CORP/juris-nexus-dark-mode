...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-vade-dark text-white flex flex-col">
      <header className="sticky top-0 z-50 shadow-md backdrop-blur-md bg-vade-darker/80 border-b border-white/5">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <Tabs 
            defaultValue="search" 
            value={viewMode} 
            onValueChange={(val) => setViewMode(val as "search" | "all")} 
            className="ml-auto"
          >
            <TabsList className="bg-vade-darker/80 border border-white/10">
              <TabsTrigger 
                value="search" 
                className="data-[state=active]:bg-vade-purple-primary data-[state=active]:text-white"
              >
